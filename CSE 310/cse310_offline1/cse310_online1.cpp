...
public:
    ...
    void PrintByType(string s)
    {
        Node* tmp =head;

        while(tmp!=0){

            if(compareString(tmp->val.getType(),s)){
...
public:
    ...
    void LookUpByType(string s)
    {

        for(int i=0;i<rowSize;i++){
            cout<<"Now in row"<<i<<endl;
            table[i].PrintByType(s);
            cout<<endl;
        }
